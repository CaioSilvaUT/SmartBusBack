...
class CartaoModel {
  async create(req, res) {
    ...
    try {
      ...
      if (results.length > 0) {
        return res
          .status(400)
          .json({ error: "Já existe um cartão para este usuário." });
      }
  ...
  async debitar(req, res) {
    ...
    try {
      ...
      await connection.query("UPDATE cartoes SET valor = ? WHERE id = ?", [
        novoSaldo,
        idCartao,
      ]);
      ...
      res
        .status(200)
        .json({ message: "Débito realizado com sucesso!", cardId: idCartao });
    } catch (err) {
      console.error(err);
      res.status(500).json({ error: "Erro interno do servidor" });
    }
  ...
  async processarSolicitacao(req, res) {
    ...
    const queryUpdateStatus =
      "UPDATE solicitacoes_cartao SET status = ? WHERE id = ?";
    ...
    try {
      ...
      if (status === "aprovado") {
        const queryGetSolicitacao =
          "SELECT idUser, tipo FROM solicitacoes_cartao WHERE id = ?";
        ...
        await connection.query(queryCreateCartao, [
          idUser,
          dataCriacao,
          dataVencimento,
          valorInicial,
        ...
        if (valor) {
          const queryAddValor =
            "UPDATE cartoes SET valor = valor + ? WHERE idUser = ?";
          await connection.query(queryAddValor, [valor, idUser]);
        }

        res.json({
          message: `Solicitação aprovada, cartão criado e saldo adicionado com sucesso para o usuário ${idUser}.`,
        });
      } else {
  ...
  async adicionarSaldo(req, res) {
    ...
    if (isNaN(valorAdicionadoFloat)) {
      return res
        .status(400)
        .json({ error: "Valor inválido para adição de saldo." });
    }
    ...
    try {
      ...
      await connection.query("UPDATE cartoes SET valor = ? WHERE idUser = ?", [
        novoSaldo,
        idUser,
      ]);
      ...
      res
        .status(200)
        .json({ message: "Saldo adicionado com sucesso.", novoSaldo });
    } catch (err) {
      console.error(err);
